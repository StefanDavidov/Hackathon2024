...
async function App() {
  ...
  const handleClick = async () => {
    try{
      const response = await axios.post('/api/getPrediction', {company});
      const data = response.data.output;
      if(data){
        setTextState("Increase")
      }else{
        setTextState("Decrease")
      }
    } catch(error) {
  ...
  return (
      <Box
        ...
        <Container sx={{
          display: "flex",
          justifyContent: pictureShown ? "normal" : "center",
          alignItems: pictureShown ? "normal" : "center"
        }} disableGutters={true} maxWidth={false}>
          <Box sx={{height: "fit-content", marginLeft: (pictureShown) ? "5%" : "0%", marginTop: "5%", maxWidth: 575,}}>
            <Typography variant="h2" align="center" sx={{fontWeight: "bold", marginTop: "2rem"}}>StockSight</Typography>
            <Typography variant="h4" align="center" sx={{marginTop: "1rem"}}> A revolutionary new system for analyzing
              stocks</Typography>
            ...
            <Container sx={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center"}}
              ...
              <FormControl sx={{minWidth: "10em", marginTop: '2em'}}>
                ...
                >
                  ...
                  <MenuItem value={"TSLA"}>Tesla</MenuItem>3
          ...
          {
            (pictureShown) ?
                <Container sx={{width: "calc(100vw - 800px)", justifyContent: "center",}} disableGutters={true}
                           maxWidth={false}>
